...
| `get23Txt(path, id, cache)` | `/src/js/get23_loadTxts.js` | async | Loads and parses a 23andMe TXT/ZIP source. Set `cache=false` to bypass LocalForage reads/writes. |
...
const parsedA = await sdk.get23Txt(pathOrUrl, "hu123ABC");

// Disable cache for this call only
const parsedB = await sdk.get23Txt(pathOrUrl, "hu123ABC", false);
```
